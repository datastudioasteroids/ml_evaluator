...
class ModelEnsembleFactory:
    ...
    def create_individual_models(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        if self.task_type == 'regression':
            self.models['random_forest'] = RandomForestRegressor(**rf_params, random_state=self.random_state)
        else:
            self.models['random_forest'] = RandomForestClassifier(**rf_params, random_state=self.random_state)
        # XGBoost
        if XGB_AVAILABLE:
            if optimize_hyperparams:
                xgb_params = self._optimize_xgboost(X_arr, y_arr, n_trials)
            else:
                xgb_params = self._get_default_xgb_params()
            if self.task_type == 'regression':
                self.models['xgboost'] = xgb.XGBRegressor(**xgb_params, random_state=self.random_state)
            else:
                self.models['xgboost'] = xgb.XGBClassifier(**xgb_params, random_state=self.random_state)
        # LightGBM
        if LGB_AVAILABLE:
            if optimize_hyperparams:
                lgb_params = self._optimize_lightgbm(X_arr, y_arr, n_trials)
            else:
                lgb_params = self._get_default_lgb_params()
            if self.task_type == 'regression':
                self.models['lightgbm'] = lgb.LGBMRegressor(**lgb_params, random_state=self.random_state)
            else:
                self.models['lightgbm'] = lgb.LGBMClassifier(**lgb_params, random_state=self.random_state)
        # Entrenar
        for name, model in self.models.items():
            model.fit(X_arr, y_arr)
        return self.models

    def create_optimized_ensemble(self, X_train: Union[pd.DataFrame, np.ndarray],
        ...
        if not self.models:
            self.create_individual_models(pd.DataFrame(X_arr), pd.Series(y_arr), optimize_hyperparams=False)
    ...
    def evaluate_ensemble_performance(self, ensemble: Any, X_test: pd.DataFrame,
                                      y_test: pd.Series,
                                      individual_models: bool = True) -> Dict[str, Dict[str, float]]:
    ...
    def save_evaluation_results(self, results: Dict[str, Dict[str, float]], filepath: str):
        """Guarda CSV y gráficos de métricas"""
        df = pd.DataFrame.from_dict(results, orient='index')
        df.to_csv(filepath)
        # Gráficos
        ax = df.plot(kind='bar', subplots=True, layout=(1, len(df.columns)), figsize=(6*len(df.columns), 4))
        plt.tight_layout()
    ...
    # Métodos privados para optimización de hiperparámetros
    def _optimize_random_forest(self, X_train: pd.DataFrame, y_train: pd.Series, n_trials: int) -> Dict[str, Any]:
    ...
    def _optimize_xgboost(self, X_train: pd.DataFrame, y_train: pd.Series, n_trials: int) -> Dict[str, Any]:
    ...
    def _optimize_lightgbm(self, X_train: pd.DataFrame, y_train: pd.Series, n_trials: int) -> Dict[str, Any]:
        ...
        def objective(trial):
            ...
            if self.task_type == 'regression':
                model = lgb.LGBMRegressor(**params, random_state=self.random_state, n_jobs=-1, verbose=-1)
                cv = KFold(n_splits=3, shuffle=True, random_state=self.random_state)
                score = cross_val_score(model, X_train, y_train, cv=cv, scoring='r2').mean()
            else:
    ...
    def _get_default_lgb_params(self) -> Dict[str, Any]:
        """Parámetros por defecto para LightGBM"""
        return {
            'n_estimators': 100,
            'max_depth': 6,
            'learning_rate': 0.1,
    ...
    def _calculate_diversity_weights(self, X_train: pd.DataFrame, y_train: pd.Series) -> Dict[str, float]:
        """Calcula pesos basados en diversidad de modelos"""
        # Implementación simplificada - en práctica se puede usar medidas de diversidad más sofisticadas
        n_models = len(self.models)
        base_weight = 1.0 / n_models
        
        # Ajustar pesos basados en el tipo de modelo (heurística simple)
        weights = {}
        for name in self.models.keys():
            if 'xgboost' in name or 'lightgbm' in name:
                weights[name] = base_weight * 1.1  # Boosting suele ser más efectivo
            elif 'random_forest' in name:
                weights[name] = base_weight * 0.9   # RF como complemento
            else:
...
class CustomEnsemble(BaseEstimator):
    ...
    def predict(self, X):
        """Hace predicciones usando el ensemble"""
...
if __name__ == "__main__":
    ...
    print("🚀 Ejemplo de uso de ModelEnsembleFactory")
